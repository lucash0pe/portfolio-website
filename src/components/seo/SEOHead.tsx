...
export function SEOHead({ 
  ...
  const fullTitle = title 
    ? `${title} | ${aboutMe.name}` 
    : `${aboutMe.name} - ${aboutMe.tagline}`;
  
  const defaultDescription = aboutMe.heroIntroduction;
  ...
  useEffect(() => {
    ...
    updateMetaTag('og:site_name', aboutMe.name, true);
    ...
    // Additional SEO tags
    updateMetaTag('author', aboutMe.name);
    updateMetaTag('keywords', `photography, ${aboutMe.name}, professional photographer, ${aboutMe.tagline}`);
